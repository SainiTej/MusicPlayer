...
app.get('/artists', (req, res, next) => {
  ...
  {
      spotifyApi.searchAlbums(req.query.input)
      .then(
        function(data) {
  ...
  {
      spotifyApi.searchTracks( req.query.input ).then( function ( data )
      {
        res.render('tracks', {tracks: data.body.tracks.items, album: req.query.input, artist: req.query.input})
      }, function ( err )
      {
        console.error( err );
      } );
